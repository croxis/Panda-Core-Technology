...
class IcosaederData(GeometryData):
    ...
    def getVertex(self, i):
        ...
        # print "ico", v.length()
        return [self.vertices[i][0] * self.radius,
...
class SphereData(GeometryData):
    def __init__(self, radius, widthSegments=32, heightSegments=17, angle=360,
                 inverted=False):
        GeometryData.__init__(self)
        self.radius = radius
...
class CircleData(GeometryData):
    def __init__(self, outerRadius=1.0, innerRadius=0.0, radiusSegments=32,
                 inverted=False):
        GeometryData.__init__(self)
        ...
        for i in xrange(radiusSegments):
            ...
            self.uv.append([x * innerRadius / outerRadius / 2. + .5,
                            y * innerRadius / outerRadius / 2. + .5])
...
# ------------------------------------------------------------------------------
# STRIP GEOMETRY GENERATOR
# ------------------------------------------------------------------------------
class StripGeometry(NodePath):
    def __init__(self):
        NodePath.__init__(self, 'Geometry')
...
class Cylinder(Geometry):
    def __init__(self, radius, length, radiusSegments):
        Geometry.__init__(self)
        side = self.addGeometry(TubeWallData(radius=radius, length=length,
                                             radiusSegments=radiusSegments))
        bottom = self.addGeometry(
            CircleData(outerRadius=radius, innerRadius=0.0,
                       radiusSegments=radiusSegments, inverted=True))
        top = self.addGeometry(CircleData(outerRadius=radius, innerRadius=0.0,
                                          radiusSegments=radiusSegments))
        top.setZ(length)


class Circle(Geometry):
    def __init__(self, outer_radius=1.0, inner_radius=0.0, radius_segments=16):
        Geometry.__init__(self)
        self.addGeometry(
            CircleData(outerRadius=outer_radius, innerRadius=inner_radius,
                       radiusSegments=radius_segments))


class Tube(Geometry):
    def __init__(self, outerRadius=1.0, innerRadius=0.5, length=1.0,
                 radiusSegments=32):
        Geometry.__init__(self)
        cylinderTop = self.addGeometry(
            CircleData(outerRadius=outerRadius, innerRadius=innerRadius,
                       radiusSegments=radiusSegments))
        cylinderTop.setZ(length)
        cylinderBottom = self.addGeometry(
            CircleData(outerRadius=outerRadius, innerRadius=innerRadius,
                       radiusSegments=radiusSegments, inverted=True))
        cylinderInnerWall = self.addGeometry(
            TubeWallData(radius=innerRadius, length=length,
                         radiusSegments=radiusSegments, inverted=True))
        cylinderOuterWall = self.addGeometry(
            TubeWallData(radius=outerRadius, length=length,
                         radiusSegments=radiusSegments))
...
class Capsule(Geometry):
    def __init__(self, radius=1.0, length=1.0, radiusSegments=32,
        ...
        side = self.addGeometry(TubeWallData(radius=radius, length=length,
                                             radiusSegments=radiusSegments,
                                             inverted=False))
        topSphere = self.addGeometry(
            SphereData(radius=radius, widthSegments=radiusSegments,
                       heightSegments=heightSegments, angle=180))
        topSphere.setZ(length)
        bottomSphere = self.addGeometry(
            SphereData(radius=radius, widthSegments=radiusSegments,
                       heightSegments=heightSegments, angle=180))
...
class Pyramid(Geometry):
    def __init__(self, radius=1.0, sides=4):
        Geometry.__init__(self)
        topSphere = self.addGeometry(
            SphereData(radius=radius, widthSegments=sides + 1,
                       heightSegments=3, angle=180))
        bottom = self.addGeometry(
            CircleData(outerRadius=radius, innerRadius=0.0,
                       radiusSegments=sides + 1, inverted=True))
...
class Sphere(Geometry):
    def __init__(self, radius=.5, segements=16):
        ...
        sphere = self.addGeometry(
            SphereData(radius=radius, widthSegments=segements,
                       heightSegment=segements, angle=360))
